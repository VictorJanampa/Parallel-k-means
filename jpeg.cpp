...
void Image::setPixel( size_t x, size_t y,u_int8_t r,u_int8_t g,u_int8_t b)
{
    if ( y >= m_bitmapData.size() )
    {
        throw std::out_of_range( "Y value too large" );
    ...
    {
        if(n==0){
            m_bitmapData[ y ][ x * m_pixelSize + n ]  = r;
        }else if(n==1){
            m_bitmapData[ y ][ x * m_pixelSize + n ]  = g;
        }else if(n==2){
            m_bitmapData[ y ][ x * m_pixelSize + n ]  = b;
        }
    }
}
